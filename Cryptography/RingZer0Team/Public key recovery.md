https://ringzer0ctf.com/challenges/50
```
 -----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----


Public key = md5(base64 blob only) MD5 2 first bytes are 0x42f5 
```

I saved the message as "privateKey.pem".

more about [pem](https://www.lifewire.com/pem-file-4147928)

To figure out the public key, run following command.

<tt>$openssl rsa -in privateKey.pem -pubout</tt>

Then, we got the public key.
In this challenge, it said public key= md5(base64 blob only).
Copy only base64 blob(exclude first and last line) and compute it's md5 digest message as follows:

<tt>$echo MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDwkrxVrZ+KCl1cX27SHDI7EfgnFJZ0qTHUD6uEeSoZsiVkcu0/XOPbz1RtpK7xxpKMSnH6uDc5On1IEw3A127wW4Y3Lqqwcuhgypd3Sf/bH3z4tC25eqr5gA1sCwSaEw+yBxdnElBNOXxOQsST7aZGDyIUtmpouI1IXqxjrDx2SQIDAQAB | tr -d '\n' | md5sum</tt>

We got
>42f51df8b6a2bafa824c179a38066e5d

First bytes matches with the given.(0x42f5) 

Alright! submit key and get the flag. 

<tt>FLAG-9869O2dQ43d1r116kfD0Sj5n</tt>
